...
### Added
- **More path navigation chords (`*`, `^`, `}`)** - Three new chords on the results pane (and in search mode) build on `>` / `<` for finer-grained query rewrites. `*` iterates over the nearest array index in the path (`.users[2].tags[1]` becomes `.users[2].tags[]`); pushes to the undo ring like `>` so `<` reverses it. `^` steps up one level by parsing the trailing path segment of the typed query and dropping the last step (`.users[0].name` becomes `.users[0]`, then `.users`, then `.`); pipe-aware so a `>`-then-`^` cleanly walks back through a drill chain; ring-free. `}` wraps the cursor's leaf as a single-entry object literal (`.users[0].name` becomes `.users[0] | {name}`, producing `{"name": "alice"}` instead of just `"alice"`); long-form syntax for keys that need bracket notation. Like `>` and `<`, all are pure textarea rewrites — execution flows through the existing debounced async pipeline. Bottom-border hints (`> value · < back · * iterate · ^ parent · } wrap`) advertise the full chord set, with `< back` shown only when the undo ring is non-empty. The `[` and `]` keys are reserved for future sibling navigation.
